...
class BaseController(object):
    ...
    def __init__(self,
        ...
        self._test_losses = {}
        self._test_accuracies_in_train = {}
        self._test_losses_in_train = {}
        self._test_consensus_errors = []
        self._test_avg_model_accuracies = {}
        self._test_avg_model_losses = {}
        self._test_in_train_avg_model_accuracies = {} #对应
        self._test_in_train_avg_model_losses = {}
        self._avg_model_genes = {}
        self._max_gene = {}
        self._test_node_num = []
    ...
    def start(self, model, nodes, graph_, agg_class, attack_class):
        ...
        logger.info("AVG Generalization: {}".format(rounding(self._avg_model_genes["avg"], 6)))
        logger.info("MAX Generalization: {}".format(rounding(self._max_gene, 6)))
    ...
    def distribution_to_test_decentralized(self):
        ...
        #对应
        uploaded_accuracies_in_train = {}
        uploaded_losses_in_train = {}
        uploaded_data_sizes_in_train = {}
        uploaded_avg_model_accuracies_in_train = {}
        uploaded_avg_model_losses_in_train = {}
        uploaded_avg_model_data_sizes_in_train = {}
        ...
        for node in test_nodes:
            ...
            #每个节点自己在训练集上test
            uploaded_content = node.run_test_in_train(self._compressed_model[node.cid], self.conf)
            uploaded_accuracies_in_train[node.cid] = uploaded_content[metric.TEST_ACCURACY_IN_TRAIN]
            uploaded_losses_in_train[node.cid] = uploaded_content[metric.TEST_LOSS_IN_TRAIN]
            uploaded_data_sizes_in_train[node.cid] = uploaded_content[metric.TEST_DATA_SIZE_IN_TRAIN]
            ...
            #增加average model在train set上做test
            uploaded_content = node.run_avg_model_test_in_train(controller_avg_model, self.conf)
            uploaded_avg_model_accuracies_in_train[node.cid] = uploaded_content[metric.TEST_ACCURACY_IN_TRAIN]
            uploaded_avg_model_losses_in_train[node.cid] = uploaded_content[metric.TEST_LOSS_IN_TRAIN]
            uploaded_avg_model_data_sizes_in_train[node.cid] = uploaded_content[metric.TEST_DATA_SIZE_IN_TRAIN]

        self.set_node_uploads_test(uploaded_accuracies, uploaded_losses, uploaded_consensus_error,
                                   uploaded_data_sizes)
        self.set_node_uploads_test_in_train(uploaded_accuracies_in_train, uploaded_losses_in_train, uploaded_consensus_error,
                                   uploaded_data_sizes_in_train)
        self.set_node_avg_model_uploads_test(uploaded_avg_model_accuracies, uploaded_avg_model_losses)
        self.set_node_avg_model_uploads_test_in_train(uploaded_avg_model_accuracies_in_train, uploaded_avg_model_losses_in_train)
    ...
    def set_node_uploads_test_in_train(self, accuracies_in_train, losses_in_train, consensus_error=0, test_sizes=1, metrics=None):
        """Set testing results uploaded from nodes.
        ...
        Args:
            ...
            metrics (dict): Node testing metrics.
        """
        self.set_node_uploads(metric.TEST_ACCURACY_IN_TRAIN, accuracies_in_train)
        self.set_node_uploads(metric.TEST_LOSS_IN_TRAIN, losses_in_train)
        self.set_node_uploads(metric.TEST_DATA_SIZE_IN_TRAIN, test_sizes)
        # self.set_node_uploads(metric.TEST_CONSENSUS_ERROR, consensus_error)
...
#对应部分
    def set_node_avg_model_uploads_test_in_train(self, test_in_train_avg_model_accuracies, test_in_train_avg_model_losses):
        """
         Set testing results on controller avg model uploaded from nodes.

        Args:
            test_avg_model_accuracies (dict[float]): Testing accuracies of nodes on controller average model.
            test_in_train_avg_model_losses (dict[float]): Testing losses of nodes on controller average model.
        """
        self.set_node_uploads(metric.TEST_IN_TRAIN_AVG_MODEL_ACCURACY, test_in_train_avg_model_accuracies)
        self.set_node_uploads(metric.TEST_IN_TRAIN_AVG_MODEL_LOSS, test_in_train_avg_model_losses)
    ...
    def init_train_test_track(self):
        ...
        for node in self._nodes:
            ...
            self._gene[node.cid] = [0]
            self._test_accuracies[node.cid] = []
            self._test_losses[node.cid] = []
            #添加
            self._test_accuracies_in_train[node.cid] = []
            self._test_losses_in_train[node.cid] = []
            self._test_avg_model_accuracies[node.cid] = []
            self._test_avg_model_losses[node.cid] = []
            #对应
            self._test_in_train_avg_model_accuracies[node.cid] = []
            self._test_in_train_avg_model_losses[node.cid] = []
            self._avg_model_genes[node.cid] = []
        ...
        #对应
        self._test_in_train_avg_model_accuracies["avg"] = []
        self._test_in_train_avg_model_losses["avg"] = []
        self._avg_model_genes["avg"] = []
        self._max_gene = []
        if self.conf.node.calculate_static_regret:
    ...
    def save_train_test_data(self):
        record = {'dataset': self.conf.data.dataset,
                  ...
                  #添加
                  metric.TEST_LOSS_IN_TRAIN: self._test_losses_in_train,
                  metric.TEST_ACCURACY_IN_TRAIN: self._test_accuracies_in_train,
                  ...
                  #对应
                  metric.GENE: self._gene,
                  metric.MAX_GENE: self._max_gene,
                  metric.TEST_IN_TRAIN_AVG_MODEL_ACCURACY: self._test_in_train_avg_model_accuracies,
                  metric.TEST_IN_TRAIN_AVG_MODEL_LOSS: self._test_in_train_avg_model_losses,
                  #添加指标
                  metric.AVG_MODEL_GENE: self._avg_model_genes,
    ...
    def init_title(self):
        ...
        self.title = "{}_{}_p{}_{}_h{}_b{}_{}_{}_{}_lr{}_{}_mo{}_{}_class_{}_dir={}_wd{}".format(
            self.conf.controller.attack_type[:4],
            self.conf.graph.graph_type[:3], centralized[:3],
            self.conf.graph.connectivity,
            ...
            self.conf.data.class_per_node,
            self.conf.data.alpha,
            self.conf.node.optimizer.weight_decay
        )
    ...
    def track_test_results(self, test_time):
        ...
        losses_avg_model = 0
        #对应
        acc_avg_model_in_train = 0
        losses_avg_model_in_train = 0
        ...
        for node in self._nodes:
            if node in self.selected_nodes and node.cid in self.graph_.honest_nodes:
                ...
                #添加
                self._test_accuracies_in_train[node.cid].append(test_metric[metric.TEST_ACCURACY_IN_TRAIN][node.cid])
                self._test_losses_in_train[node.cid].append(test_metric[metric.TEST_LOSS_IN_TRAIN][node.cid])
                ...
                #对应
                self._test_in_train_avg_model_accuracies[node.cid].append(test_metric[metric.TEST_IN_TRAIN_AVG_MODEL_ACCURACY][node.cid])
                self._test_in_train_avg_model_losses[node.cid].append(test_metric[metric.TEST_IN_TRAIN_AVG_MODEL_LOSS][node.cid])
                acc += test_metric[metric.TEST_ACCURACY][node.cid]
                losses += test_metric[metric.TEST_LOSS][node.cid]
                acc_in_train += test_metric[metric.TEST_ACCURACY_IN_TRAIN][node.cid]
                losses_in_train += test_metric[metric.TEST_LOSS_IN_TRAIN][node.cid]
                ...
                #对应
                acc_avg_model_in_train += test_metric[metric.TEST_IN_TRAIN_AVG_MODEL_ACCURACY][node.cid]
                losses_avg_model_in_train += test_metric[metric.TEST_IN_TRAIN_AVG_MODEL_LOSS][node.cid]
                ...
                self._test_accuracies_in_train[node.cid].append(0)
                self._test_losses_in_train[node.cid].append(0)
                self._test_avg_model_accuracies[node.cid].append(0)
                self._test_avg_model_losses[node.cid].append(0)
                #对应
                self._test_in_train_avg_model_accuracies[node.cid].append(0)
                self._test_in_train_avg_model_losses[node.cid].append(0)
        ...
        #对应
        self._test_in_train_avg_model_accuracies["avg"].append(acc_avg_model_in_train / test_node_num)
        self._test_in_train_avg_model_losses["avg"].append(losses_avg_model_in_train / test_node_num)
        ...
        # if self._current_round == 0:
        #     self._avg_model_genes["avg"].append(0)
        # else:
        #     self._avg_model_genes["avg"].append(abs(losses_avg_model_in_train -losses_avg_model) / test_node_num)
        # while True:
        #     random_node = random.choice(self.selected_nodes)
        #     if random_node.cid in self.graph_.honest_nodes:
        #         break
        # self._avg_model_genes["avg"].append(abs(self._test_avg_model_accuracies[random_node.cid][-1] -self._test_in_train_avg_model_accuracies[random_node.cid][-1]))
        self._avg_model_genes["avg"].append(abs(losses_avg_model_in_train -losses_avg_model) / test_node_num)
        ...
        if self.controller_retain_one_model:
            ...
            self.print_('AVG Model Test loss in Train: {:.6f}, Test accuracy: {:.2f}%'.format(
                self._test_in_train_avg_model_losses["avg"][-1], self._test_in_train_avg_model_accuracies["avg"][-1] * 100))
            #对应
            self.print_('AVG Model Generalization Error: {:.6f}'.format(
                self._avg_model_genes["avg"][-1]))
        # self.wandb_log()

# #改到这里
#这里的trian loss不是eval模式的，可能也需要更改
    def track_gene_results(self):  
        """
        Record each node train accuracy, loss and regret to the self._.
        """
        max_gene=0
        ...
        for node in self._nodes:
            if node in self.selected_nodes and node.cid in self.graph_.honest_nodes:
                gene = abs(self._test_losses_in_train[node.cid][-1]-self._test_losses[node.cid][-1])
                self._gene[node.cid].append(gene)
                if gene>max_gene:
                    max_gene = gene
            else:
                self._gene[node.cid].append([0])
        # if self._current_round == 0:
        #     max_gene=0
        self._max_gene.append(max_gene)
        self.print_('Max Generalzation Error: {:.6f}'.format(
                self._max_gene[-1]))
        self.wandb_log()
    ...
    def wandb_log(self):
        ...
        if self.should_print() and self.conf.wandb_param.use_wandb:
            ...
            # self.wandb_message["Train loss"] = self._train_losses["avg"][-1]
            self.wandb_message["Test loss in Train"] = self._test_in_train_avg_model_losses["avg"][-1]
            self.wandb_message["Test loss"] = self._test_avg_model_losses["avg"][-1]
            if self.conf.node.calculate_static_regret:
                self.wandb_message["Train regret"] = self._train_regrets["avg"][-1]
            if not self.controller_retain_one_model:
                self.wandb_message["Max Generalzation Error"] = self._max_gene[-1]
                self.wandb_message["AVG Model Generalization Error"] = self._avg_model_genes["avg"][-1]
                ...
                # #对应
                # self.wandb_message["AVG Model Test loss in Train"] = self._test_in_train_avg_model_losses["avg"][-1]
                # self.wandb_message["AVG Model Test accuracy in Train"] = self._test_in_train_avg_model_accuracies["avg"][-1] * 100
