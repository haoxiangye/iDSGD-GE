...
from src.datas.make_data.make_mnist import get_mnist
...
class BaseNode(object):
    ...
    def __init__(self,
        ...
        self.test_in_trainset_data = test_in_trainset_data
        self.test_in_trainset_loader = None
        ...
        self.test_loss = 0
        self.test_accuracy_in_train = 0
        self.test_loss_in_train = 0
        self.current_regret = 0
        self.test_data_size = 1
        self.test_data_size_in_train = 1
    ...
    #改动
    def run_test_in_train(self, model, conf):
        """Conduct testing on nodes.
        ...
        self.pre_test()
        self.test_in_trainset(conf, self.device)
        self.post_test()
    ...
    #average model在train set上做test
    def run_avg_model_test_in_train(self, model, conf):
        """Conduct testing on nodes.
        ...
        self.pre_test()
        self.test_in_trainset(conf, self.device, another_model=model)
        self.post_test()
    ...
    def train_iteration(self, conf, device=metric.CPU):
        ...
        Args:
            ...
            device (str): Hardware device for training, cpu or cuda devices.
        """
        # def project_model_onto_ball(model, radius=10):
        #     # 将所有参数拼接成一个长向量来计算整体范数
        #     with torch.no_grad():
        #         # 将模型的所有参数展平并拼接为一个大向量
        ...
        #                 param.copy_(param * scale)  # 按比例缩放每个参数
        # # def projection_onto_ball(y, radius):
        # #     norm_y = torch.norm(y)
        # #     if norm_y > radius:
        # #         return y * (radius / norm_y)
        # #     return y
        ...
        for i in range(conf.node.local_iteration):
            ...
            batched_x, batched_y = next(self.train_loader)
            #改动
            # self.sample_x= batched_x
            # self.sample_y= batched_y
            if str(batched_y.dtype) == "torch.int32" or str(batched_y.dtype) == "torch.int16":
                batched_y = batched_y.type(torch.LongTensor)
            x, y = batched_x.to(device), batched_y.to(device)
            # #改动
            self.sample_x= x
            self.sample_y= y
            # self.sample_x, self.sample_y = batched_x.to(device), batched_y.to(device)          
            optimizer.zero_grad()
            out = self.model(x)
            loss = loss_fn(out, y)
            loss.backward()
            ...
            if i == conf.node.local_iteration - 1 and self.conf.node.message_type_of_node_sent == "gradient":
                ...
                optimizer.step()
                # with torch.no_grad():
                #     project_model_onto_ball(self.model, radius=10)
            _, y_pred = torch.max(out, -1)
            correct += y_pred.eq(y.data.view_as(y_pred)).long().cpu().sum()
    ...
    #增加在训练集上测试average model的模块(使用自定义loader) (每个节点的训练集版本)
    def test_in_trainset(self, conf, device=metric.CPU, another_model=None):
        """Execute node testing.
        ...
        model.to(device)
        loss_fn = self.load_loss_fn(conf)
        if self.test_in_trainset_loader is None:
            self.test_in_trainset_loader = self.test_in_trainset_data
        # TODO: make evaluation metrics a separate package and apply it here.
        self.test_loss_in_train = 0
        correct = 0
        with torch.no_grad():
            batched_x, batched_y = next(self.test_in_trainset_loader)
            if str(batched_y.dtype) == "torch.int32" or str(batched_y.dtype) == "torch.int16":
                batched_y = batched_y.type(torch.LongTensor)
            x, y = batched_x.to(device), batched_y.to(device)
            log_probs = model(x)
            ...
            _, y_pred = torch.max(log_probs, -1)
            correct += y_pred.eq(y.data.view_as(y_pred)).long().cpu().sum()
            self.test_loss_in_train = loss.item()
            total = len(batched_y)
            self.test_data_size_in_train = len(batched_y)
            self.test_accuracy_in_train = float(correct) / self.test_data_size_in_train

        logger.debug('Node {}, testing in train -- Average loss: {:.8f}, Accuracy: {}/{} ({:.2f}%)'.format(
            self.cid, self.test_loss_in_train, correct, self.test_data_size_in_train, self.test_accuracy_in_train * 100))
    ...
    def construct_upload_request(self):
        ...
        try:
            if self._is_train:
                ...
                #增加训练集的数据部分
                uploads[metric.TEST_ACCURACY_IN_TRAIN] = self.test_accuracy_in_train
                uploads[metric.TEST_LOSS_IN_TRAIN] = self.test_loss_in_train
                uploads[metric.TEST_DATA_SIZE_IN_TRAIN] = self.test_data_size_in_train
        except KeyError:
