...
class Coordinator(object):
    ...
    def __init__(self):
        ...
        self.test_data = None
        self.test_in_trainset_data =None
        self.test_in_trainset_iter=None,
        self.val_data = None
        self.conf = None
        self.model = None
    ...
    def init_dataset(self):
        ...
        if self.registered_dataset:
            return
        self.train_data_iter, self.test_data_iter, self.test_in_trainset_iter, self.extra_info = construct_federated_datasets(
        ...
        # self.test_in_trainset_data  = self.extra_info["test_in_trainset_iter"]
...
def init_wandb(conf, config):
    ...
    if config.wandb_param.use_wandb:
        ...
        project_name = config.wandb_param.project_name if config.wandb_param.project_name != "" \
            else "my_{}_{}_{}_{}_{}_{}_{}_wd{}".format(config.task_name, config.data.dataset, config.model, is_centralized, config.data.partition_type,config.node.lr_controller,config.lr_controller_param.init_lr,config.node.optimizer.weight_decay)

        wandb.init(project=project_name,
                   config=conf,
                   name="{}_{}_{}_n={}_b={}".format(config.controller.aggregation_rule, conf["controller"]["attack_type"],
                                          config.graph.graph_type[:4],config.graph.nodes_cnt,config.graph.byzantine_cnt))
