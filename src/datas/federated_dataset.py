...
def construct_partition_datasets(
    ...
    # load partition
    partition_type_dict_class = {"iid": "IIDPartition", "fixiid": "FIXIIDPartition","fixnoniid": "NonIIDFIXSeparation", "fixdir":"DirichletFixNonIID","successive": "SuccessivePartition",
    ...
    partition_type_dict_continuous = {"iid": "IIDPartition", "fixiid": "FIXIIDPartition","fixnoniid": "NonIIDFIXSeparation","fixdir":"DirichletFixNonIID" ,"successive": "SuccessivePartition",
...
def construct_federated_datasets(dataset_name,
    ...
    # test_generator = eval("{}_generator".format(generator))
    # get_test_iter = partial(test_generator,
    #                          batch_size=train_batch_size)
    # test_data_iter = [get_test_iter(dataset=test_par_set[node]) for node in range(num_of_par_nodes)]
    ...
    get_test_iter = partial(test_generator)
    test_data_iter = [get_test_iter(dataset=test_par_set[node], batch_size=train_batch_size)
                      for node in range(num_of_par_nodes)]
    if dataset_name == 'Mnist':
        # test_in_trainset_generator = full_generator
        # get_test_in_trainset_iter = partial(full_generator)
        # test_in_trainset_iter = [get_test_in_trainset_iter(dataset=data.train_set) for node in range(num_of_par_nodes)]
        test_in_trainset_generator = random_generator
        get_test_in_trainset_iter = partial(random_generator,batch_size=60000)   ##test in train 
        test_in_trainset_iter = [get_test_in_trainset_iter(dataset=train_par_set[node]) for node in range(num_of_par_nodes)]
    if dataset_name == 'Cifar10':
        test_in_trainset_generator = random_generator
        get_test_in_trainset_iter = partial(random_generator,batch_size=train_batch_size)
        test_in_trainset_iter = [get_test_in_trainset_iter(dataset=train_par_set[node]) for node in range(num_of_par_nodes)]
    # extra_info["test_in_trainset_iter"] = [test_in_trainset_iter]
    # test_data_iter = [get_test_iter(dataset=test_par_set[node], batch_size=int(len(test_par_set[node]) * test_rate))
    #                   for node in range(num_of_par_nodes)]

    return train_data_iter, test_data_iter, test_in_trainset_iter, extra_info



# def construct_test_in_trainset(dataset_name,):
#     test_in_trainset = partial(full_generator)